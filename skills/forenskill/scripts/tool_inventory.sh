...
# -ver, poppler uses -v, most GNU tools use --version), so each is tried in
# turn and the first output line that looks like a version is kept.

tool_version() {
  local t="$1" flag out
  for flag in --version -V -version -ver -v; do
    out=$(timeout 5 "$t" "$flag" </dev/null 2>&1 | grep -a -m1 -E '[0-9]+\.[0-9]+')
    [ -n "$out" ] && { printf '%s\n' "$out"; return; }
  done
  echo "present (version not detected — record manually)"
}

for t in mmls fsstat fls icat istat tsk_recover blkls blkstat ifind img_stat \
         ewfmount ewfinfo affuse xmount \
         exiftool file binwalk foremost scalpel photorec bulk_extractor \
         7z unzip zipinfo pdftotext pdfinfo xxd strings \
